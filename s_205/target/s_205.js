// Transcrypt'ed from Python, 2019-06-27 09:11:17
...
export var draw = function () {
	...
	background (242);
	for (var [i, y] of enumerate (range (0, height, 25))) {
		var index = (frameCount + i) * 0.07;
		var max_x = map (noise (index), 0, 1, 0, width);
		stroke (27);
		for (var x = 0; x < max_x; x += 10) {
			line (x, y, x - 10, y + 20);
		}
		fill (27);
		noStroke ();
		...
		endShape (CLOSE);
		stroke (242);
		for (var x = max_x; x < width + 10; x += 10) {
			line (x - 10, y, x, y + 20);
		}
	}
};
