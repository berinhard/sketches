// Transcrypt'ed from Python, 2019-06-16 20:06:47
...
export var y_ranges = list ([]);
export var squares = list ([]);
export var Square =  __class__ ('Square', [object], {
	...
	get display () {return __get__ (this, function (self) {
		...
		push ();
		var r = map (self.angle, TWO_PI, 0, 0, self.size);
		if (self._revert) {
			var r = self.size - r;
		}
	...
	get py_update () {return __get__ (this, function (self) {
		...
		var prev_angle = self.angle;
		self.angle = __mod__ (frameCount * self.rotation_speed, TWO_PI);
		self.x += self.speed;
		if (prev_angle <= self.angle) {
			self._revert = !(self._revert);
		}
	});},
	get _get_is_appearing () {return __get__ (this, function (self) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
...
Object.defineProperty (Square, 'is_appearing', property.call (Square, Square._get_is_appearing));;
...
export var draw = function () {
	...
	if (!(__mod__ (frameCount, 50))) {
		var y = choice (y_ranges);
