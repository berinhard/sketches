// Transcrypt'ed from Python, 2019-05-12 19:10:45
...
export var draw = function () {
	noFill ();
	var row_size = 100;
	var x_pos = enumerate (range (0, width + row_size, row_size));
	var y_pos = enumerate (range (0, height + row_size, row_size));
	for (var [i, x] of x_pos) {
		for (var [j, y] of y_pos) {
			stroke (27, 27, 27, 5);
			if (frameCount == 1) {
				rect (x, y, row_size, row_size);
			}
			var r = noise (i, j, frameCount * 0.004) * row_size;
			ellipse (x, y, r, r);
		}
	}
	print (frameCount);
